...
class LinkedList(object):
    ...
    def insert_at_index(self, index, item):
        ...
        while (current != None):
            print("here",prev_node, current)
            if ll_index == index:
                old_current_item = current
                current = new_node
                current.next = old_current_item
                # prev_node.next = current
                # prev_node.next = new_node
                if (0 < ll_index <= self.size):
                    prev_node.next = current
                if index == 0:
                    self.head = current
                ...
                return
            # checks for the ending being appended
            if self.size == index and current.next == None:
